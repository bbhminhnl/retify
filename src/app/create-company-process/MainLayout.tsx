...
const MainLayout = () => {
  ...
  const [on_reset, setOnReset] = useState(false);
  const resetForm = () => {
    setFormData((prev) => {
      return { ...DEFAULT_FORM_DATA, access_token: prev.access_token };
    });
    setOnReset(true);
  };
      ...
      {!loading_init && (
        <div className="flex flex-col items-center justify-center px-3 py-5 gap-4 w-full md:max-w-[400px] md:mx-auto bg-white h-full">
          ...
          {open_modal_connect_shopify && (
            <ConnectShopify
              loading={loading}
              closeModal={() => setOpenModalConnectShopify(false)}
              is_open={open_modal_connect_shopify}
              setLoading={setLoading}
              chatbox_token={form_data.access_token}
